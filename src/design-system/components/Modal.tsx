// Canonical modal backdrop and bounded dialog surface. Feature dialogs own
// their content and actions; this primitive owns viewport placement, elevation,
// responsive bounds, and background interaction blocking.
import {
  forwardRef,
  type HTMLAttributes,
  type ReactNode,
} from "react";
import { createPortal } from "react-dom";

export function ModalBackdrop({
  children,
  ...props
}: {
  children: ReactNode;
} & Omit<HTMLAttributes<HTMLDivElement>, "className" | "children">) {
  return createPortal(
    <div
      className="tw:fixed tw:inset-0 tw:z-[var(--ds-z-modal)] tw:grid tw:place-items-center tw:overflow-auto tw:bg-overlay tw:p-4 tw:max-[640px]:p-2"
      role="presentation"
      {...props}
    >
      {children}
    </div>,
    document.body,
  );
}

export const ModalSurface = forwardRef<
  HTMLElement,
  {
    children: ReactNode;
    size?: "medium" | "wide";
    fill?: boolean;
  } & Omit<HTMLAttributes<HTMLElement>, "className" | "children">
>(function ModalSurface(
  { children, size = "medium", fill = false, ...props },
  ref,
) {
  return (
    <section
      ref={ref}
      role="dialog"
      aria-modal="true"
      data-size={size}
      data-fill={fill}
      className="ds-panel tw:flex tw:max-h-[calc(100dvh-(var(--ds-space-4)*2))] tw:w-[min(640px,100%)] tw:flex-col tw:overflow-hidden tw:p-0 tw:shadow-popover tw:data-[fill=true]:h-[min(760px,calc(100dvh-(var(--ds-space-4)*2)))] tw:data-[size=wide]:w-[min(1120px,100%)] tw:max-[640px]:max-h-[calc(100dvh-(var(--ds-space-2)*2))] tw:max-[640px]:data-[fill=true]:h-[calc(100dvh-(var(--ds-space-2)*2))]"
      onMouseDown={(event) => event.stopPropagation()}
      {...props}
    >
      {children}
    </section>
  );
});
