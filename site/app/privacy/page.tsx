    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "Optional Desktop product analytics: only after explicit opt-in, the app sends random installation and session identifiers, a one-way event identifier, app version, operating-system family, language, pseudonymous one-way account and workspace keys when applicable, workspace kind, and closed event outcomes such as database engine, local or managed access, statement class, provider, approval flag, role, and bucketed duration or row count.",
      ...
      },
      {
        ...
        paragraphs: [
          "Sentry error diagnostics and PostHog product analytics are separate. Sentry receives only the sanitized production error projection described above and is not used to build product funnels. Optional product analytics remains off while your choice is pending or denied and is relayed through app.dopedb.dev to the EU PostHog endpoint only after you choose to allow it.",
          "The first-party relay validates a closed event schema and does not store raw analytics in the workspace database. PostHog receives no original client IP in the event payload, uses the random installation identifier without creating a person profile, and receives no autocapture, cookies, replay, heatmaps, surveys, or free-form properties. Vercel may separately process the relay request and its IP as hosting and security data.",
          ...
          "Withdrawing Desktop product-analytics consent stops future collection and deletes the pending local queue and random installation identifier immediately. If you opt in again, the app creates new installation and session identifiers. Stable pseudonymous account and team-workspace keys may still group separately consented team events; request vendor-side deletion if you want those prior events removed. This choice does not change Sentry diagnostics or the public website's separate Vercel Web Analytics.",
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          "PostHog provides EU-region product analytics only after Desktop opt-in. DopeDB's first-party relay sends the closed event projection without the original client IP and disables person profiles and autocapture. Policy: https://posthog.com/privacy.",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Sanitized production Desktop errors are sent directly to Sentry's United States ingest service. Optional Desktop product analytics first reaches the Vercel-hosted workspace relay in the United States and is then sent to PostHog's European Union endpoint without the original client IP in the analytics payload.",
      ...
      },
      {
        ...
        items: [
          ...
          "The optional Desktop analytics retry queue contains at most 100 closed events, discards events older than seven days, and is deleted with its random installation identifier when consent is withdrawn. Raw PostHog EU product events are retained for no more than 12 months; only non-identifying aggregate counts may be retained longer. Sanitized Sentry events are retained only for the configured incident-investigation period and are deleted or anonymized when no longer needed.",
      ...
      },
      {
        ...
        items: [
          ...
          "You can withdraw optional Desktop product-analytics consent in the app. This immediately clears its local queue and installation identifier. Contact us before resetting if you also want deletion assistance for already relayed pseudonymous events. Because DopeDB does not keep an account mapping for the random installation identifier, we may be unable to locate installation-only events after it is deleted; stable pseudonymous account or team-workspace keys may still let us locate applicable team events. Sentry and Vercel requests are handled separately because their identifiers are not joined to the Desktop analytics identifier.",
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "선택형 데스크톱 제품 분석: 명시적 동의 후에만 임의 생성한 설치·세션 식별자, 일방향 이벤트 식별자, 앱 버전, 운영체제 종류, 언어, 필요한 경우 계정·워크스페이스의 일방향 가명 키, 워크스페이스 종류, DB 엔진·로컬 또는 관리형 접근·명령 종류·Agent 공급자·승인 여부·역할·구간화한 실행 시간이나 행 수 등 폐쇄형 결과를 전송합니다.",
      ...
      },
      {
        ...
        paragraphs: [
          "Sentry 오류 진단과 PostHog 제품 분석은 서로 별개입니다. Sentry에는 위에서 설명한 정식 앱의 정제된 오류 정보만 전송하며 제품 퍼널을 만드는 데 사용하지 않습니다. 선택형 제품 분석은 선택이 미정이거나 거부된 동안 꺼져 있고, 이용자가 허용한 뒤에만 app.dopedb.dev의 중계 서버를 거쳐 EU PostHog 엔드포인트로 전송됩니다.",
          "중계 서버는 폐쇄형 이벤트 스키마를 검증하고 원본 분석 이벤트를 워크스페이스 DB에 저장하지 않습니다. PostHog 이벤트에는 원래 이용자의 IP가 포함되지 않고, 임의 설치 식별자를 이용하되 개인 프로필을 만들지 않으며 자동 수집, 쿠키, replay, heatmap, 설문 또는 자유 형식 속성을 사용하지 않습니다. Vercel은 중계 요청과 IP를 별도의 호스팅·보안 정보로 처리할 수 있습니다.",
          ...
          "데스크톱 제품 분석 동의를 철회하면 향후 수집이 즉시 중단되고 전송 대기열과 임의 설치 식별자가 로컬에서 삭제됩니다. 다시 동의하면 새 설치·세션 식별자를 만듭니다. 안정적인 가명 계정·팀 워크스페이스 키는 별도로 동의한 팀 이벤트를 계속 묶을 수 있으므로 이전 이벤트까지 삭제하려면 공급자 측 삭제를 요청해야 합니다. 이 선택은 별도 Sentry 진단이나 공개 웹사이트의 Vercel Web Analytics 설정을 변경하지 않습니다.",
        ],
      },
      {
        title: "처리 목적과 법적 근거",
      ...
      },
      {
        ...
        items: [
          ...
          "PostHog는 데스크톱에서 명시적으로 동의한 경우에만 EU 리전 제품 분석을 제공합니다. DopeDB 중계 서버는 원래 이용자의 IP 없이 폐쇄형 이벤트만 전송하고 개인 프로필과 자동 수집을 비활성화합니다. 방침: https://posthog.com/privacy.",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "정제된 정식 데스크톱 오류는 Sentry의 미국 수집 서비스로 직접 전송됩니다. 선택형 데스크톱 제품 분석은 먼저 미국의 Vercel 워크스페이스 중계 서버에 도달한 다음, 분석 이벤트에 원래 이용자의 IP를 포함하지 않은 채 PostHog의 유럽연합 엔드포인트로 전송됩니다.",
      ...
      },
      {
        ...
        items: [
          ...
          "선택형 데스크톱 분석 재전송 대기열은 폐쇄형 이벤트를 최대 100개 보관하고 7일이 지난 이벤트를 버리며, 동의를 철회하면 임의 설치 식별자와 함께 삭제됩니다. PostHog EU의 원본 제품 이벤트는 최대 12개월까지만 보유하고 이후에는 개인을 식별하지 않는 집계 수치만 더 오래 보유할 수 있습니다. 정제된 Sentry 이벤트는 설정된 장애 조사 기간에만 보유하고 더 이상 필요하지 않으면 삭제하거나 익명화합니다.",
      ...
      },
      {
        ...
        items: [
          ...
          "앱에서 선택형 데스크톱 제품 분석 동의를 철회할 수 있으며, 전송 대기열과 설치 식별자가 즉시 삭제됩니다. 이미 중계된 가명 이벤트의 삭제 지원도 원하면 식별자를 초기화하기 전에 문의해 주세요. DopeDB는 임의 설치 식별자와 계정을 연결한 표를 보관하지 않으므로 설치 전용 이벤트는 식별자 삭제 후 찾지 못할 수 있지만, 안정적인 가명 계정·팀 워크스페이스 키가 있는 팀 이벤트는 찾을 수 있습니다. Sentry와 Vercel은 Desktop 분석 식별자와 연결하지 않으므로 각 공급자 요청을 별도로 처리합니다.",
