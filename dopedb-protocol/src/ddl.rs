...
mod tests {
    ...
    #[test]
    fn request_version_is_fixed_by_constructor() {
        ...
        assert_eq!(request.schema_version, DDL_IR_SCHEMA_VERSION);
        assert_eq!(request.change.relation(), &relation());
    }

    #[test]
    fn default_change_keeps_drop_and_set_distinct_on_wire() {
        let keep = serde_json::to_value(DefaultChange::Keep).unwrap();
        let drop = serde_json::to_value(DefaultChange::Drop).unwrap();
        let set = serde_json::to_value(DefaultChange::Set {
            expression: "now()".into(),
        })
        .unwrap();
