//! Desktop SQL classification, preview, proposal, and execution, plus the
//! not-yet-migrated Terminal SQL proposal path.
//! Terminal read planning and execution live in `features::queries`; this service
//! remains transport-neutral until the proposal path moves in its own complete slice.
...
use crate::model::{
    Classification, ExecOutcome, HistoryEntry, PreviewMode, PreviewReport, QueryKind,
    SafetySettings,
};
use crate::operations::{
    actor_for_pin, agent_actor_for_pin, capture_policy, ensure_operation_scope,
    required_confirmation, NewOperation, OperationKind, OperationPlanDisposition,
