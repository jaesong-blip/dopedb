...
mod tests {
    ...
    #[tokio::test]
    async fn introspect_preserves_rich_sqlite_metadata_without_counting_rows() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        sqlx::query(
            "CREATE TABLE parents (
                a INTEGER NOT NULL,
                b INTEGER NOT NULL,
                CONSTRAINT parents_pk PRIMARY KEY (a, b)
             )",
        )
        .execute(&pool)
        .await
        ...
        sqlx::query(
            "CREATE TABLE children (
                ...
                a INTEGER NOT NULL DEFAULT 7,
                b INTEGER NOT NULL,
                label TEXT COLLATE NOCASE,
                normalized TEXT GENERATED ALWAYS AS (lower(label)) STORED,
                CONSTRAINT positive_a CHECK (a > 0),
                CONSTRAINT parent_fk FOREIGN KEY (a, b)
                  REFERENCES parents(a, b) ON UPDATE CASCADE ON DELETE RESTRICT,
                UNIQUE(label)
             )",
        )
        .execute(&pool)
        ...
        .unwrap();
        sqlx::query(
            "CREATE INDEX children_label_expr
             ON children(lower(label) DESC) WHERE label IS NOT NULL",
        )
        .execute(&pool)
        .await
        .unwrap();
        sqlx::query("INSERT INTO parents VALUES (7, 1)")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("INSERT INTO children(a, b, label) VALUES (7, 1, 'One')")
            .execute(&pool)
            .await
            .unwrap();
        ...
        assert!(table.native_id.is_some());
        let id = table
            .columns
            .iter()
        ...
        let a = table
            ...
            .unwrap();
        assert_eq!(a.default_expression.as_deref(), Some("7"));
        let generated = table
            .columns
            .iter()
            ...
            .unwrap();
        assert_eq!(
            generated.generated_expression.as_deref(),
            Some("lower(label)")
        );
        let label = table
            .columns
            ...
            .unwrap();
        assert_eq!(label.collation.as_deref(), Some("NOCASE"));
        assert_eq!(table.foreign_keys.len(), 2);
        assert_eq!(table.foreign_keys[0].name, table.foreign_keys[1].name);
        assert_eq!(table.foreign_keys[0].ordinal, 1);
        assert_eq!(table.foreign_keys[1].ordinal, 2);
        assert_eq!(
            table.foreign_keys[0].update_action.as_deref(),
            Some("CASCADE")
        );
        assert!(table.constraints.iter().any(|constraint| {
            constraint.kind == ConstraintKind::Check
                && constraint.name == "positive_a"
                && constraint.check_expression.as_deref() == Some("a > 0")
        }));
        assert!(table.constraints.iter().any(|constraint| {
            constraint.kind == ConstraintKind::Unique
                && constraint.columns == vec!["label".to_string()]
        }));
        let index = table
            .indexes
            .iter()
            .find(|index| index.name == "children_label_expr")
            .unwrap();
        ...
        assert_eq!(
            ...
            vec![IndexKey {
                column: None,
                expression: Some("lower(label)".into()),
