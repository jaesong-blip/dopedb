...
mod tests {
    ...
    use super::{
        append_bounded, neon_url, parse_neon_read_endpoint, valid_neon_host,
        validate_neon_database, validate_response_meta, MAX_PROVIDER_BODY_BYTES,
    };

    #[test]
    fn neon_resolution_requires_one_exact_database_and_enabled_read_write_endpoint() {
        assert!(validate_neon_database(&json!({"databases":[{"name":"app"}]}), "app").is_ok());
        assert!(parse_neon_read_endpoint(&json!({"endpoints":[{"type":"read_write","disabled":false,"host":"ep-green.neon.tech"}]})).is_ok());
        for value in [
            json!({"endpoints":[{"type":"read_write","disabled":false,"host":"evil.example"}]}),
            json!({"endpoints":[{"type":"read_write","disabled":false,"host":"one.neon.tech"},{"type":"read_write","disabled":false,"host":"two.neon.tech"}]}),
            json!({"endpoints":[{"type":"read_only","disabled":false,"host":"one.neon.tech"}]}),
            json!({"endpoints":[{"type":"read_write","host":"one.neon.tech"}]}),
        ] {
            assert!(parse_neon_read_endpoint(&value).is_err());
        }
        assert!(!valid_neon_host("user@one.neon.tech"));
    }

    #[test]
    fn neon_http_seam_rejects_redirects_and_any_declared_or_streamed_oversize_body() {
        assert!(
            validate_response_meta(StatusCode::OK, Some(MAX_PROVIDER_BODY_BYTES as u64)).is_ok()
        );
        assert!(validate_response_meta(StatusCode::FOUND, Some(2)).is_err());
        assert!(
            validate_response_meta(StatusCode::OK, Some(MAX_PROVIDER_BODY_BYTES as u64 + 1))
                .is_err()
        );
        let mut body = Vec::new();
        append_bounded(&mut body, &vec![b'x'; MAX_PROVIDER_BODY_BYTES]).unwrap();
        assert!(append_bounded(&mut body, b"x").is_err());
        assert_eq!(
            neon_url("project-one", "branch-one", "databases")
                .unwrap()
                .as_str(),
            "https://console.neon.tech/api/v2/projects/project-one/branches/branch-one/databases"
        );
        assert!(neon_url("project-one", "branch-one", "connection_uri").is_err());
    }
}
