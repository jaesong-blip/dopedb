//! Compatibility migrations for pre-versioned and legacy local stores.

use super::*;

pub(super) async fn add_legacy_columns(pool: &SqlitePool) -> AppResult<()> {
    let columns = [
...
/// `connections.db_name`.
pub(in super::super) async fn add_sql_document_database_scope(pool: &SqlitePool) -> AppResult<()> {
...
pub(in super::super) async fn migrate_agent_acp_providers(pool: &SqlitePool) -> AppResult<()> {
