...
impl Store {
    ...
    pub(crate) async fn automation_runner_device_id(&self) -> AppResult<Uuid> {
        let generated = Uuid::new_v4();
        sqlx::query(
            "INSERT INTO app_settings (key, value)
             VALUES ('analysis_runner_device_id', ?1)
        ...
        let value: String = sqlx::query_scalar(
            "SELECT value FROM app_settings WHERE key = 'analysis_runner_device_id'",
        )
        .fetch_one(&self.pool)
        .await?;
